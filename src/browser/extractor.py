...
class CredRow(QWidget):
    ...
    def _toggle_pwd(self):
        ...
        if self._pwd_visible:
            ...
            # Use Segoe UI so ALL special characters in
            # passwords render correctly. Consolas cannot
            # display many unicode symbols browsers use.
...
class VaultDashboard(QMainWindow):
    ...
    def _import_from_browser(self):
        ...
        try:
            ...
            if not summary:
                self._toast("// No supported browsers found.")
                return
            ...
            self._toast("// Scanning all browser profiles...")

            extracted = extractor.extract_all()

            if not extracted:
                self._toast(
                    "// No saved credentials found in any browser."
                )
                if extractor.errors:
                    self._err(
            ...
            if not new_creds:
                self._toast(
                    "// All browser credentials already in vault."
                )
                return
            ...
            if box2.exec() == QMessageBox.StandardButton.Yes:
                ...
                write_log(
                    f"BROWSER_IMPORT  {len(new_creds)} credentials"
                )
                self._reload_creds()
                self._toast(
                    f"// IMPORTED:  {len(new_creds)} credential"
                    f"{'s' if len(new_creds) != 1 else ''}"
                )
